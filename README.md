...
* full path to rebus binary to COS_BIN_NAME ( check ```which rebusd```)
* node PRC port to COS_PORT_RPC ( check in file ```path_to_rebus_node_config/config/config.toml```)
* node validator address to COS_VALOPER ( like ```rebusvaloper********```)

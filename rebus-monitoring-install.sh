...
echo ''
echo -e 'INSTALLING REBUS NODE MONITORING'
...
# make the telegraf user sudo and adm to be able to execute scripts as rebus user
...
#Rebus monitoring variables template 
COS_BIN_NAME=$COS_BIN_NAME             # example: /root/go/bin/rebusd or /home/user/go/bin/rebusd
COS_PORT_RPC=$COS_PORT_RPC         # default: 26657
COS_VALOPER=$COS_VALOPER           # example: rebusvaloper1234545636767376535673
EOL
...
[[outputs.influxdb]]
  database = "rebusmetricsdb"
  urls = [ "http://95.216.2.219:8086" ] 
  username = "metric" 
  password = "password" 
[[inputs.exec]]
  commands = ["sudo su -c /root/rebus-monitoring/monitor.sh -s /bin/bash root"] # change path to your monitor.sh file and username to the one that validator runs at (e.g. root)
  interval = "15s"
  timeout = "5s"
  data_format = "influx"
  data_type = "integer"
...
echo ''
echo -e '\e[7mYour Rebus node monitoring is installed!\e[0m'
echo ''
